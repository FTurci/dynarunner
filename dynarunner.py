...
class DynamoMD:
	...
	def equilibrate(self,collisions,conf=None):
		...
		os.system(f"{self.root}/dynarun -E -c {collisions} --engine=1 --config-file {conf} -o {self.equilibrated} --out-data-file {self.output}/equilibration.log.xml")
		conf_to_atom(self.equilibrated, collisions)

	def run(self,conf,collisions,snapshot=10):
		os.system(f"{self.root}/dynarun -c {collisions} --engine=1 --config-file {conf} --snapshot-events {snapshot} -o {self.output}/config.end.xml.bz2 --out-data-file {self.output}/output.end.xml.bz2")
		os.system(f"mv Snapshot* {self.output}")